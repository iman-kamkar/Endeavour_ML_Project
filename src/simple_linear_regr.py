import numpy as np
from src.simple_linear_regr_utils import generate_data, evaluate
...
class SimpleLinearRegression:
    ...
    def __loss(self, y, y_hat):
        ...
        loss = np.sum((y_hat - y)**2) / y.shape[0]
    ...
    def __sgd(self, X, y, y_hat):
        ...
        dW = 2 * np.dot(X.T, (y_hat - y)) / y.shape[0]
        db = 2 * np.sum(y_hat - y) / y.shape[0]
